...
async function promoteCommand(sock, chatId, mentionedJids, message) {
    ...
    try {
        ...
        const promotionMessage = `*『 الترويج الجماعي 』*\n\n` +
            `👥 *المستخدم الذي تمت ترقيته${userToPromote.length > 1 ? 's' : ''}:*\n` +
            `${usernames.map(name => `• ${name}`).join('\n')}\n\n` +
            `👑 *تمت ترقيته بواسطة:* @${promoterJid.split('@')[0]}\n\n` +
...
async function handlePromotionEvent(sock, groupId, participants, author) {
    try {
        ...
        const promotionMessage = `*『 الترويج الجماعي 』*\n\n` +
            `👥 *المستخدم الذي تمت ترقيته${participants.length > 1 ? 's' : ''}:*\n` +
            `${promotedUsernames.map(name => `• ${name}`).join('\n')}\n\n` +
            `👑 *تمت ترقيته بواسطة:* ${promotedBy}\n\n` +
            `📅 *تاريخ:* ${new Date().toLocaleString()}`;
