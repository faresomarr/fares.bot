...
async function demoteCommand(sock, chatId, mentionedJids, message) {
    try {
        ...
        const demotionMessage = `*『 تخفيض رتبة المجموعة 』*\n\n` +
            `👤 *المستخدم الذي تم تخفيض رتبته${userToDemote.length > 1 ? 's' : ''}:*\n` +
            `${usernames.map(name => `• ${name}`).join('\n')}\n\n` +
            `👑 *تم تخفيض رتبته بواسطة:* @${message.key.participant ? message.key.participant.split('@')[0] : message.key.remoteJid.split('@')[0]}\n\n` +
